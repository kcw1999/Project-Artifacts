Function ADHealthReport {
    ...
    # New Data Collection for Domain Controller Uptime, DNS Health, Trust Relationships, and GPO Info
    $dcUptime = Get-SafeADData { Get-ADDomainController -Filter * | ForEach-Object { 
        [PSCustomObject]@{
            DCName    = $_.Name
            Uptime    = (Get-WmiObject -ComputerName $_.Name -Class Win32_OperatingSystem).LastBootUpTime
        } 
    } }

    $dnsHealth = Get-SafeADData {
        Get-ADDomainController -Filter * | ForEach-Object {
            [PSCustomObject]@{
                DCName      = $_.Name
                DNSHealthy  = Test-Connection -ComputerName $_.Name -Count 1
            }
        }
    }
    ...
    if ($dcUptime) {
        $excelData += @{ Title = "DC Uptime"; Data = $dcUptime }
    } else {
        Write-Host "Warning: Domain Controller Uptime data is missing." -ForegroundColor Yellow
    }

    if ($dnsHealth) {
        $excelData += @{ Title = "DNS Health"; Data = $dnsHealth }
    } else {
        Write-Host "Warning: DNS Health data is missing." -ForegroundColor Yellow
    }
